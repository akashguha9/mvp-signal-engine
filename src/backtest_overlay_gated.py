...
def main() -> None:
    df = pd.read_csv("data/processed/final_dataset_with_overlay_gated.csv")
    df["Date"] = pd.to_datetime(df["Date"], errors="coerce")

    summary = pd.DataFrame([
        {"rule": "overlay_gated", **score(df, "overlay_signal_gated")}
    ])

    summary.to_csv("data/processed/overlay_gated_summary.csv", index=False)
