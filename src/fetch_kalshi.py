...
BLOCK_TERMS = {
    ...
    "grammy", "celebrity", "weather", "crosscategory"
}

BLOCK_PATTERNS = {
    "1+", "2+", "3+", "over", "under", "runs", "points", "yards", "rebounds",
    "assists", "hits", "home runs", "strikeouts", "touchdowns", "wins by"
}
...
def _is_relevant_market(title: str, subtitle: str, event_ticker: str, series_ticker: str) -> bool:
    ...
    if any(pat in text for pat in BLOCK_PATTERNS):
        return False

    # encoded junk / sports combo style
    if "crosscategory" in text:
        return False
    if "yes " in text and any(x in text for x in ["1+", "2+", "3+", "over", "under"]):
        return False
    if " vs " in text:
        return False

    # Must contain at least one macro/geopolitical signal term
...
def fetch_kalshi_markets(limit: int = MAX_KALSHI_MARKETS) -> pd.DataFrame:
    ...
    if relevant > 0:
        ...
        print(
            df[df["is_relevant"] == True][
                ["market_id", "title", "series_ticker", "event_ticker"]
            ].head(20).to_string()
        )
